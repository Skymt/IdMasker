...
public class Masker
{
    ...
    public string Mask(IEnumerable<ulong> ids, int minLength = 0, ulong increment = 0)
    {
        var firstId = ids.First();
        // If a mask contains inapropriate words, a different mask can be generated by adding an increment.
        var seed = firstId + (ulong)ids.Count() + increment;
...
// 
// The mask will thus be one character longer than the other maskers masks.
// But an upside is that only one character of the alphabet is reserved for special use.
